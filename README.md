<p align="center">
  <a href="https://github.com/SobhanRjz">
    <img src="./Banner.gif" alt="LinkedIn Banner" width="1500"/>
  </a>
</p>

# 👋 Hi, I'm Sobhan Rajabzadeh

I'm a **Software Developer** with a strong focus on **AI** and **machine learning**. I build intelligent systems that solve real-world problems — from infrastructure defect detection to structural optimization and data-driven applications.

---

## 🛠️ Skills

- **Languages:** Python, C++, C#, JavaScript
- **AI/ML:** Deep Learning, Computer Vision, Image Processing, Metaheuristic Algorithms
- **Development:** Windows Forms (C#), Web (HTML, CSS, JS), Backend (Node.js, PHP)
- **Tools:** PyTorch, TensorFlow, Keras, Docker, Git, AutoCAD API, ETABS
- **Automation & Data:** Web Scraping, Dataset Annotation, Data Augmentation, Scripting

---

## 🚀 Featured Projects

- **Automated Sewer Pipe Defect Detection**  
  AI-powered system to detect defects in sewer pipes using deep learning and computer vision.

- **Detect-Sewer-Defects**  
  Deep learning application for identifying sewer system defects to support maintenance decisions.

- **SewerAi_WinForm**  
  Windows Forms app to visualize and manage sewer inspection data using integrated AI models.

- **DL_Detectron2**  
  Object detection using Detectron2, demonstrating advanced CV capabilities.

- **Make-Sense**  
  Image annotation tool to create and manage datasets for machine learning tasks.

- **Shear Wall Optimization Module**  
  ETABS add-on to optimize rebar layout, improving material efficiency in structural design.

- **ETABS Civil Engineering Plugin**  
  Automates compliance checks and structural procedures in civil engineering workflows.

- **Crypto Intelligence Pipeline**  
  Extracts market signals and features for AI-driven trading models.

---

## 📫 Contact

- 📧 Email: Sobhan.rajabzadeh@gmail.com  
- 🌐 LinkedIn: [sobhan-rajabzadeh](https://www.linkedin.com/in/sobhan-rajabzadeh)

---

Thanks for visiting!
